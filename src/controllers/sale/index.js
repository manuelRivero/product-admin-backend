...
const saveSaleByNotification = async (req, res) => {
  ...
  if (topic === "payment") {
    ...
    try {
      ...
      await sendSuccessEmail({user, names: `${name} ${last_name}`, products: products.map((product) =>({ quantity: product.quantity, data: {...product}})), total, tenant: sub_domain, payment_id: id})
